...
export default function ConceptOneHundred({ onContinue }) {
  ...
  const scriptParagraphs = [
    'Cosmic humility balances confidence in effort with reverence for scale and uncertainty.',
    'You act fully while accepting that you are one participant in a vast living system.',
    'Humility is not self-erasure; it is accurate perspective under mystery.',
    'From this stance, service deepens and ego reactivity softens.',
  ];
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-slate-950 text-slate-100 flex flex-col items-center justify-center p-6 sm:p-12 font-sans">
      ...
      <Module25Progress step={4} label="Cosmic Humility" />
      <LyricalAnchor line={module25LyricLines.concept100} targetTime={module25LyricTimings.concept100} />

      <motion.div className="relative max-w-3xl space-y-7 text-base sm:text-lg leading-relaxed text-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1 }}>
        <p className="text-xs uppercase tracking-[0.24em] text-fuchsia-100/80">Concept 100 // Cosmic Humility</p>
